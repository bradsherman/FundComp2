...
class Puzzle {
	...
	private:
		...
		vector< vector< set<T> > > Possibilities;
};
#endif
...
Puzzle<T>::Puzzle() {
	...
	for (i = 0; i < boardSize; i++) {
		possibilities.push_back( vector< set<T> > (9));
		for (j = 0; j < boardSize; j++) {
				myfile >> tmp;
				board[i][j] = tmp;
			for( k = 0; k < 9; k++ ) {
				possibilities[i][j].insert(k+1);
			}
		}
	}
		...
		for( i = 0; i < boardSize; i++ ) {
			for( j = 0; j < boardSize; j++ ) {
				if( Board[i][j] == 0 ) {
					...
					for( auto h = 0; h < 9; h++ ) {
						if( checkMiniGrids( h+1, i, j ) == 0 ) Possibilities[i][j].erase(h+1);
					}
					...
					for( h = 0; h < boardSize; h++ ) {
						if( Possibilities[i][j].count(h+1) == 1 ) {
							num = h+1;
							count++;
						}	
					}
...
void Puzzle<T>::checkRow( int row, int col ) {
	...
	for( n = 0; n < boardSize; n++ ) {
		if( Board[row][n] != 0 ) {
			auto entry = Board[row][n];
			Possibilities[row][col].erase(entry);
		}
	}	
}
template <typename T>
